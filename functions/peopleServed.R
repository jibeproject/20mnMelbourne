...
# There are 2 versions, the first without parallel processing and the second with
# Parallel processing may fail depending on computer power, especially for 
# the park layer.  Also, it's not much faster unless there are large numbers 
# (eg when running for existing destinations).


# version 1, without parallel processing - useful in case of memory problems
peopleServed <- function(destinations,
  ...
  # for (i in 1:length(destination.types)) {
  for (i in c(13:length(destination.types), 1:12)) {  # alt - starting with parks
...
# # version 2, using parallel processing - faster, but may encounter memory issues 
...
#   
#   # for (i in 1:length(destination.types)) {
#   for (i in c(14:length(destination.types), 1:12)) {  # all except park
#     
#     # report progress
#     print(paste(Sys.time(), "|", destination.types[i], "- loading destinations"))
#     
...
#     cores <- detectCores()  # or -
#     # cores <<- min(detectCores(), 4)  # if using full no of cores is causing issues with parks
